...
OutDir="Output/" # Folder were annotations will be saved
...
MatIOUthresh=0.3#5# IOU quality threshold for predicted material instance to be accepted
...
OutSem=OutDir+"/SemanticAnn/" # Semantic annotation results
OutIns=OutDir+"/InstanceAnn/" # Instance annotations results
OutImg=OutDir+"/Image/"
OutVizSem=OutDir+"/SemanticVisual/" # Semantic annotations overlay on image for vizuallization
OutVizIns=OutDir+"/InstanceVisual/" # instance annotations overlay on image for visuallization


#----make ouput dirs--------
if not os.path.exists(OutDir): os.makedirs(OutDir)
if not os.path.exists(OutSem): os.makedirs(OutSem)
if not os.path.exists(OutIns): os.makedirs(OutIns)
if not os.path.exists(OutImg): os.makedirs(OutImg)
if not os.path.exists(OutVizSem): os.makedirs(OutVizSem)
if not os.path.exists(OutVizIns): os.makedirs(OutVizIns)
print("Loading nets")

OutSem2=OutDir+"/Semantic2/"
if not os.path.exists(OutSem2): os.makedirs(OutSem2)
#=========================Load Semantic net====================================================================================================================
...
for name in os.listdir(InputDir): # Scan and annotate all images in input dir
    print(name)
    AnnName=name[:-4]
    CatDic[AnnName]={}
    CatDic[AnnName]["PartCats"] = {}  # categories of each part instance
    CatDic[AnnName]["MaterialCats"] = {}  # Categories of each material intace
    InPath=InputDir+"/"+name
    Im=cv2.imread(InPath)
    cv2.imwrite(OutImg+"/"+AnnName+".jpg",Im)
...
#==============Create and save annotation map for each class====================================================================================================
    ...
    for nm in OutLbDict:
        ...
        CatMap[nm]=Lb
        if not os.path.exists(OutSem+"/"+nm): os.mkdir(OutSem+"/"+nm)
        cv2.imwrite(OutSem+"/"+nm+"/"+name[:-4]+".png",cv2.resize(Lb.astype(np.uint8),(w0,h0),interpolation=cv2.INTER_NEAREST))
#*****************************Create  visualization of the semantic map, display map on image and save in OutVizSem ************************************************************************************************
        if not os.path.exists(OutVizSem + "/" + nm): os.mkdir(OutVizSem + "/" + nm)
        ImOverlay1 = Im.copy()
        ImOverlay2 = Im.copy()
        ImOverlay1[:, :, 0] *= 1 - Lb
        ImOverlay2[:, :, 1] *= 1 - Lb
        ImOverlay2[:, :, 2] *= 1 - Lb
        FinIm=np.concatenate([ImOverlay1,Im,ImOverlay2],axis=1)
        OutPath = OutVizSem + "//" + nm+"/"

        if not os.path.exists(OutPath): os.makedirs(OutPath)
        OutName=OutPath+name[:-4]+".png"
        cv2.imwrite(OutName,FinIm)
        print("Saving semantic map " + OutName)
#***************************************************************************************************************************************
        if  nm=='Filled': break
...
#####################################################Instance segmentation#####################################################################################################
 #------------------------Find Vessel instance take the vessel region find in the semantic segmentation and split it into individual vessel instances--------------------------------------------------------------------------------------------------------------------
    ...
    OutInstVesDir = OutIns + "/" + name + "/"

#***********************************************************************************************************************************************************************************************************


    # for ff, VIns in enumerate(VesInsList):
    #              Im2=Im.copy()
    #              Im2[:, :, 0] *= 1 - VIns.astype(np.uint8)
    #              Im2[:, :, 1] *= 1 - VIns.astype(np.uint8)
    #              misc.imshow(Im2)

#***********************************************************************************************************************************************************************************************************

    # if not os.path.exists(OutInstVesDir): os.makedirs(OutInstVesDir)
    # VesInsViz=np.concatenate([np.expand_dims(InstMapVes * 40, axis=2),np.expand_dims(InstMapVes * 40, axis=2),np.expand_dims(InstMapVes * 40, axis=2)],axis=2)
    # VesInsViz = np.concatenate([Imgs[0],VesInsViz],axis=1)
    # cv2.imwrite(OutInstVesDir+"/VesselInstance.png",VesInsViz)
...
# ***********************************************************************************************************************************************************************************************************
       ...
       # for ff, VIns in enumerate(MatInsList):
       #     Im2 = Im.copy()
       #     Im2[:, :, 0] *= 1 - VIns.astype(np.uint8)
       #     Im2[:, :, 1] *= 1 - VesIns.astype(np.uint8)
       #     misc.imshow(Im2)
...
#-------------------------------------Save second type semantic maps--------------------------------------------------------------------------------------
    for nm in CatMap2:
       if (nm in MaterialCats) and (nm in CatMap2.keys()):
           if not os.path.exists(OutSem2 + "/" + nm): os.mkdir(OutSem2 + "/" + nm)
           cv2.imwrite(OutSem2 + "/" + nm + "/" + name[:-4] + ".png",cv2.resize(CatMap2[nm].astype(np.uint8),(w0,h0),interpolation=cv2.INTER_NEAREST))
    #-----------------------------------Save json file----------------------------------------------------------------------------------------------------------------------------------
    import json
    with open(OutDir+'//InstCategory.json', 'w') as fp:
    ...
    print("Saving instance map "+OutVizIns)
    VizInstImg=np.concatenate([InsVizVessl, Im, InsVizMat,InsVizVesPart], axis=1).astype(np.uint8)
    cv2.imwrite(OutVizIns+"/"+AnnName+".png", VizInstImg)
    print("-----------------------------------------------Finish image-------------------------------------------------")
